...
from engine.programme_year import canonical_programme_year, programme_year_report_value, programme_year_report_value_from_source
...
def build_programme_completeness_rows(
    ...
    for assignment in assignments:
        if assignment.is_fixed and assignment.room is not None and assignment.timeslot is not None:
            fixed[programme_year_report_value_from_source(assignment.course.prog_yr, assignment.course.source_file)] += 1
    for item in quarantined:
